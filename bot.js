...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "rarest") {
    ...
    if (!sorted.length) {
      return interaction.reply({ content: "❌ No data found for that filter.", ephemeral: true });
    }
  ...
  if (commandName === "ban") {
    ...
    if (!guild.members.me.permissions.has(PermissionFlagsBits.BanMembers))
      return interaction.reply({ content: "❌ I don't have Ban Members permission.", ephemeral: true });
    ...
    if (targetMember) {
      if (targetMember.roles.highest.position >= interaction.member.roles.highest.position)
        return interaction.reply({ content: "❌ You can't ban someone with equal or higher role.", ephemeral: true });
      if (targetMember.roles.highest.position >= guild.members.me.roles.highest.position)
        return interaction.reply({ content: "❌ That member has a higher role than me.", ephemeral: true });
    }
