...
const { BASELINE_DATA } = require("./data");
...
// ─── In-memory cache — pre-loaded with baseline so bot works instantly ───────
let CACHE = { ...BASELINE_DATA };
let META_CACHE = { lastScrape: null, lastSuccess: null, totalEntries: Object.keys(BASELINE_DATA).length, lastLog: null };
...
function loadData() {
  ...
  try { CACHE = JSON.parse(fs.readFileSync(DATA_FILE, "utf8")); return CACHE; }
  catch { return {}; }
}
...
function saveMeta(meta) {
  META_CACHE = meta;
  try { fs.writeFileSync(META_FILE, JSON.stringify(meta, null, 2)); } catch(_) {}
}
